...
static bool check_data_dir(char *data_dir);
...
static char *generate_random_name(char *prefix);

static int find_other_exec_version(const char *argv0, const char *target,
...
main(int argc, char **argv)
{
	...
	static struct option long_options[] = {
		...
		{"publication-names", required_argument, NULL, 3},
	...
	while ((c = getopt_long(argc, argv, "D:n:sv", long_options, &optindex)) != -1)
	{
		switch (c)
		{
			...
			case 2:
				...
				break;
			case 3:
				publication_names = validate_publication_names_input(pg_strdup(optarg));
				break;
			case 4:
				{
	...
	if (!publication_names || !strlen(publication_names))
		die(_("Publication names string must be specified.\n"));
	 ...
	 * initialized.
	 */
	logfd = open("pg_create_subscriber_postgres.log", O_CREAT | O_RDWR,
				 S_IRUSR | S_IWUSR);
	if (logfd == -1)
	{
		die(_("Creating pg_create_subscriber_postgres.log failed: %s"),
			strerror(errno));
	}
	...
	restore_point_name = generate_random_name("pg_create_subscriber");
	...
	/*
	 * Start subscriber and wait until it starts accepting connections which
	 * means it has caught up to the restore point.
	 */
	pg_ctl_ret = run_pg_ctl("start -l \"pg_create_subscriber_postgres.log\" -o \"-c shared_preload_libraries=''\"");
	if (pg_ctl_ret != 0)
		die(_("Postgres startup for restore point catchup failed with %d. See pg_create_subscriber_postgres.log."), pg_ctl_ret);
	...
	if (pg_ctl_ret != 0)
		die(_("Postgres stop after restore point catchup failed with %d. See pg_create_subscriber_postgres.log."), pg_ctl_ret);
	wait_postmaster_shutdown();

	/*
	 * Start the node again, now we can start the logical replication.
     * This is final start, so don't log to to special log file anymore.
	 */
	print_msg(VERBOSITY_NORMAL,
			  _("Initializing subscriber node:\n"));

	pg_ctl_ret = run_pg_ctl("start");
	if (pg_ctl_ret != 0)
		die(_("Postgres restart failed with %d."), pg_ctl_ret);
...
usage(void)
{
	printf(_("%s create new logical subscriber from basebackup of provider.\n\n"), progname);
	printf(_("Usage:\n"));
	printf(_("  %s [OPTION]...\n"), progname);
	printf(_("\nGeneral options:\n"));
 ...
 */
static char *
validate_publication_names_input(char *publication_names)
{
	char	   *name;
...
static char *
generate_random_name(char *prefix)
{
	char *rn = malloc(NAMEDATALEN);

    if (prefix == NULL)
        prefix = "random_name";
	
    snprintf(rn, NAMEDATALEN-1, "%s_%lx", prefix, random());
	return rn;
}
